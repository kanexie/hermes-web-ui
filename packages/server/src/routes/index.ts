...
import { knowledgeBaseRoutes } from './hermes/knowledge-base'
...
export function registerRoutes(app: any, authMiddleware: Array<(ctx: Context, next: Next) => Promise<void>>) {
  ...
  app.use(knowledgeBaseRoutes.routes())      // Must be before proxy
  ...
  app.use(writeGateRoutes.routes())              // Hermes Agent write approval review  app.use(proxyRoutes.routes())
